...
**エンジニア職務経歴（2020-現在 | Python歴5年 Javascript歴 3年 | 転職活動中）**:
- **AWS Bedrock API開発**、Django/Flask/FastAPI での実務経験
- **1000万行超のデータ分析**、1億レコード規模のDB設計・最適化
- 要件定義〜運用まで一貫して担当できるフルサイクル開発経験
- **テストは苦手です**

**現職・株式会社グレイプでの開発（生成AIをフル活用）**:
- **社内CMSの設計・構築**（要件定義〜運用までフルサイクルで担当）
- **社内向けAIチャットツールの開発**（業務効率化）
- **BigQuery × 生成AI のデータ分析ツール開発**（現在進行中）
...
**個人開発**:
- **[スロマップAI](https://slo-map.com)**: 25年8月MVPリリース。**月間35万PV / 登録会員 約5,000人 / 有料プラン稼働中（課金ユーザー約50人）**。Google AdSense + 月額サブスクで収益化
...
- 🛠️ **SEOの作り込み（得意領域）** — GSC API + Serper API で検索順位・CTRを継続トラッキングしデータ駆動でPDCA。6,000+ページのプログラマティックSEO（動的 sitemap/robots・クロールバジェット最適化）、タイトル/メタをデータ駆動で動的生成（多段フォールバック）、Core Web Vitals 対策まで一貫
